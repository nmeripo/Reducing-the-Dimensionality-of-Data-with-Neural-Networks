...
# Autoencoder
autoencoder = AutoEncoder(784, [1000, 500, 250, 30], [['rbmW_1', 'rbmb_1'],
                                                      ['rbmW_2', 'rbmb_2'],
                                                      ['rbmW_3', 'rbmb_3'],
                                                      ['rbmW_4', 'rbmb_4']], symmetric_weights=True)
...
for i in range(nb_epoch):
    ...
    print "Epoch:", i, "Cost: ", rbm_3.compute_cost(rbm_2.transform(rbm_1.transform(X_train)))
...
for i in range(nb_epoch):
    ...
    print "Epoch:", i, "Cost:", rbm_4.compute_cost(rbm_3.transform(rbm_2.transform(rbm_1.transform(X_train))))
...
plt.scatter(autoencoder.transform(X_test)[:, 0], autoencoder.transform(X_test)[:, 1], alpha=0.5)
plt.show()
