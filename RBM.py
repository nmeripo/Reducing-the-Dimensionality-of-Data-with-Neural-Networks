...
class RBM(object):
    def __init__(self, n_visibe, n_hidden, layer_names, learning_rate=0.01):
        ...
        self.hprobs_0, self.hstates_0, self.visible_1, self.hprobs_1 = self._gibbs_sampling_step()
        ...
        # sampling functions
        self.h_sample = tf.nn.sigmoid(tf.matmul(self.visible_0, self.rbm_W) + self.rbm_b)
    ...
    def _initialize_weights_and_biases(self):
        weights_and_biases = {
            ...
            'W': tf.Variable(tf.random_normal([self.n_visible, self.n_hidden], stddev=0.01, dtype=tf.float32),
            ...
            'a': tf.Variable(tf.zeros([self.n_visible], dtype=tf.float32), name=self.layer_names[1]),
            'b': tf.Variable(tf.zeros([self.n_hidden], dtype=tf.float32), name=self.layer_names[2])
        }
    ...
    def _gibbs_sampling_step(self):
        ...
        visible_1 = tf.nn.sigmoid(tf.matmul(hprobs_0, tf.transpose(self.rbm_W)) + self.rbm_a)
    ...
    def _update_weights_and_biases(self):
        update_W = self.rbm_W + self.learning_rate * (self.positive_grad - self.negative_grad) / tf.to_float(
            tf.shape(self.visible_0)[0])
        update_a = self.rbm_a + self.learning_rate * tf.reduce_mean(self.visible_0 - self.visible_1, 0)
        update_b = self.rbm_b + self.learning_rate * tf.reduce_mean(self.hprobs_0 - self.hprobs_1, 0)
        return update_W, update_a, update_b

    def restore_weights(self, path):
        saver = tf.train.Saver({self.layer_names[0]: self.weights_and_biases['W'],
    ...
    def compute_cost(self, batch):
        return self.sess.run(self.error, feed_dict={self.visible_0: batch, self.rbm_W: self.o_w,
                                                    self.rbm_a: self.o_vb, self.rbm_b: self.o_hb})

    def partial_fit(self, batch_x):
        #  It is often more efficient to divide the training set into small “mini-batches” of 10 to 100 cases
        self.n_w, self.n_vb, self.n_hb = self.sess.run([self.update_W, self.update_a, self.update_b],
                                                       feed_dict={self.visible_0: batch_x, self.rbm_W: self.o_w,
                                                                  self.rbm_a: self.o_vb, self.rbm_b: self.o_hb})
        ...
        return self.sess.run(self.error,
                             feed_dict={self.visible_0: batch_x, self.rbm_W: self.n_w, self.rbm_a: self.n_vb,
                                        self.rbm_b: self.n_hb})

    def transform(self, batch_x):
        return self.sess.run(self.h_sample, {self.visible_0: batch_x, self.rbm_W: self.o_w,
                                             self.rbm_a: self.o_vb, self.rbm_b: self.o_hb})
